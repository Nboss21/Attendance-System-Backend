...
export class AuthService {
	...
	private async issue(user: any) {
		...
		const accessToken = this.jwt.sign(
			claims,
			Number(ENV.JWT_ACCESS_TTL_SECONDS || 900),
		);
		const refreshToken = randomBytes(48).toString("base64url");
		const refreshTtl = Number(ENV.JWT_REFRESH_TTL_SECONDS || 2592000);
		...
		return {
			...
			refreshToken,
			tokenType: "Bearer",
			expiresIn: Number(ENV.JWT_ACCESS_TTL_SECONDS || 900),
			user: {
				id: user.id,
				email: user.email,
	...
	async logout(accessToken: string, refreshToken?: string) {
		...
		await this.blacklist.blacklist(payload.jti, payload.exp);
		if (refreshToken)
